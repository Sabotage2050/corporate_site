...
const Example = ({ gasoline_stocklist, slug, fork_kind }) => {
  //console.log(list);
  ...
  return (
    <div className="my-8">
      <div className=" grid grid-cols-3 gap-4">
        {gasoline_stocklist.map((stocklist, index) => (
          <Stocklist
            key={slug + index}
            stocklist={stocklist}
            index={index}
...
export function getStaticProps() {
  ...
  const csv_forks = files.map((fileName, index) => {
    ...
    const data = fs.readFileSync(`csv/gasoline_photos.csv`);
  ...
  // let diesel_stocklist = arr[1];
  // let shovelloader_stocklist = arr[3];
  let gasoline_stocklist = arr[0];
  // //console.log(gasoline_stocklist);
  // //表示するstocklistを取得
  ...
  // let present_gasoline_list = [];
  // gasoline_stocklist.map((element, index) => {
  //   present_list.map((element2, index) => {
  //     for (let w of element) {
  //       if (w === element2[0]) {
  //         present_gasoline_list.push(element);
  //       }
  //     }
  //   });
  // });
  // //console.log(present_list);
  // present_gasoline_list = new Set(present_gasoline_list);
  // //console.log(present_gasoline_list);
  // present_gasoline_list = Array.from(present_gasoline_list);
  //
  // //imageと番号を紐付け
  // const image_file_names = fs.readdirSync("public/stocklist");
  // //console.log(image_file_names);
  // let image_number_list = [];
  // present_gasoline_list.map((element, index) => {
